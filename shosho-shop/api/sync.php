...
foreach ($sales as $sale) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO sales (sale_uuid, total_amount, payment_method, cash_received, change_given, status, sold_by) VALUES (UUID(), ?, ?, ?, ?, 'synced', ?)");
        $stmt->execute([$sale['total_amount'], $sale['payment_method'], $sale['cash_received'] ?? 0, $sale['change_given'] ?? 0, $_SESSION['user_id']]);
